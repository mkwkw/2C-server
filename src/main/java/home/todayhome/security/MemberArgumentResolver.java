...
public class MemberArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        ...
        Map<String, String> decodedToken = jwtService.decode(token); //Map 형태의 decoded 토큰 얻기
        String userEmail = decodedToken.get(JwtService.CLAIM_NAME_USER_EMAIL);
        if(userEmail==null){
            throw new RuntimeException("UnauthorizedException");
        }

        return AuthInfo.of(userEmail); //AuthInfo에 값 바인딩
    }
}
